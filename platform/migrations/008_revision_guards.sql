begin;
-- Keep implementations private and validate nullable RPC arguments before executing them.
alter function club_app.submit_score(uuid,uuid,int,int,int) rename to submit_score_impl;
alter function club_app.assign_courts(uuid,uuid,int,jsonb,int,text) rename to assign_courts_impl;
alter function club_app.complete_session(uuid,uuid,int) rename to complete_session_impl;
alter function club_app.cancel_session(uuid,uuid,int) rename to cancel_session_impl;
revoke all on function club_app.submit_score_impl(uuid,uuid,int,int,int),club_app.assign_courts_impl(uuid,uuid,int,jsonb,int,text),club_app.complete_session_impl(uuid,uuid,int),club_app.cancel_session_impl(uuid,uuid,int) from public,anon,authenticated,service_role;
create function club_app.submit_score(c uuid,m uuid,a int,b int,expected_revision int) returns int language plpgsql security definer set search_path='' as $$ begin
...
create function club_app.assign_courts(c uuid,s uuid,round_number int,plan jsonb,expected_revision int,reason text) returns void language plpgsql security definer set search_path='' as $$ begin
 if expected_revision is null or expected_revision<0 then raise exception 'Valid revision required';end if;
...
create function club_app.complete_session(c uuid,s uuid,expected_revision int) returns void language plpgsql security definer set search_path='' as $$ begin
 if expected_revision is null or expected_revision<0 then raise exception 'Valid revision required';end if;
...
revoke all on function club_app.submit_score(uuid,uuid,int,int,int),club_app.assign_courts(uuid,uuid,int,jsonb,int,text),club_app.complete_session(uuid,uuid,int),club_app.cancel_session(uuid,uuid,int),club_app.import_permit(uuid,uuid,uuid,text,text,text,jsonb,int,numeric,boolean) from public,anon;
grant execute on function club_app.submit_score(uuid,uuid,int,int,int),club_app.assign_courts(uuid,uuid,int,jsonb,int,text),club_app.complete_session(uuid,uuid,int),club_app.cancel_session(uuid,uuid,int),club_app.import_permit(uuid,uuid,uuid,text,text,text,jsonb,int,numeric,boolean) to authenticated;
commit;
