...
| p68 — players are set before the session starts; the organizer's own vote locks | "Spares cannot be called in middle of a session, all player numbers will be finalized before a session begin" (Call In, Attendance's "Seat anyway", the "Spares for Session" card's "🪑 Seat" and the Players tag all seated players during a session); "I am still able to vote no … its past Sun 10.00 pm" (the 46-hour lock skipped the organizer's own answer) | `tabs/call-in.spec.ts` (12: no Call In or "Seat anyway" during a session, "Not playing tonight", a call-in refused with tonight's courts unchanged; 8: players seated tonight show where they play); `tabs/admin-extras.spec.ts` (the Players tag of a player without a court tonight is refused); `tabs/organizer-vote.spec.ts` (8: open before the deadline, locked after it, changed in Standings → RSVP); `tabs/complete-locks.spec.ts`; `unit/players-set.test.mjs` (6) |
| p70 — no Seat during a session | Attendance's "Marked present but not on a court" offered "🪑 Seat", which put the player on a court during the session ("remove Seat too") | `tabs/attendance-count.spec.ts` (40: no Seat; "Not here" clears each mark; tonight's courts unchanged); the button census |
| p69 — spares fill the courts up to 24, decided when the regulars' vote closes | One spare seat per declined regular (27 regulars, 4 declines → 4 spares), reserved as soon as a spare answered; the organizer's rule is 24 players, decided after the regulars' deadline, in reply order, confirmed once paid | `unit/spare-seats.test.mjs` (7); `tabs/oracle.ts` spareSeatCount (the reference for starting courts and spare charges); `tabs/vote.suite.ts`; `tabs/call-in.spec.ts` (16 spare cases before and after the deadline); `opener.spec.ts` |
...
## Before a production release: spare seats in the database and the reminder emails

p69 changed the page to the organizer's rule (24 players, decided when the regulars' vote closes). The database's
`spare_seats` and reminder functions (migration L07) and the reminder emails still count one spare seat per declined
regular and invite spares when a regular declines. The TEST reminder job does not email players, and production runs
the pinned earlier code, so nothing is sent under either rule today. Before a production release they must follow the
same rule (a migration and the reminder job), or the spare emails must be switched off since the organizer invites
spares personally.
