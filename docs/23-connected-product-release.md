...
- **80/80 desktop/mobile browser cases passed** in the final clean GitHub run, alongside all 1,190 unit/database tests, lint, build and the dependency audit: [quality run 34385904619](https://github.com/Chriz93/dcbadmintonclub-test/actions/runs/34385904619), application source commit `23eb549b63ebe756b8394dbc20ece8c4c2e3a738`. CI caught an empty-season title regression introduced by the initial heading refinement; it was fixed, and first-load failure/recovery now has explicit coverage on both devices. These browser cases use mocked API responses; they are not independent real Auth tests.
