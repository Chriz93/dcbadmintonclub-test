...
- **p90 — seating a regular puts them back in tonight's line-up, and the message tells the truth** (the organizer, 22 September: "I TRIED adding Rahul Tamarkar to court 4, it didnt work ... when a high level spare comes, I bring a down a player of lower caliber so thats why Rahul lost his spot on court 3, but then when I try to add him to court 4, its not wokring"): taking a regular off a court before a session marks them **not coming** (`removeFromUpcoming` — no penalty, they keep the court they earned), and `autoAssign` drops everyone who is not coming **before** it reads anybody's court. So seating Rahul on Court 4 wrote `current_court=4` and changed nothing on screen: he stayed excluded, Court 4 stayed empty, and he appeared on no court at all. p89's new message made it worse by claiming "Rahul is on Court 4" while the grid showed him nowhere — it reported the request instead of the result. Now putting a regular on a court is taken to mean they are playing, so it lifts a "not coming" vote or an absent mark exactly as the organizer's own Present mark does (`pre_session_attendance`), and the message names the court the player actually landed on — including "on Court 5, not Court 4 — the courts are settled so nobody plays alone" when the rules put them elsewhere. Test: `tabs/seat-brings-back.spec.ts`; on the unfixed page the first case finds the player on no court at all and the second catches the false claim.

- **p91 — the court panel stops repeating itself, and the page says which build it is** (the organizer, 22 September, screenshotting the Court 4 panel headed "Court 4 — Court 4": "what is this"): courts 1–3 carry a medal name, and courts 4–6 have none, so the badge fell back to the court's own name and the title printed it twice. The title now shows the medal where there is one and the plain court name where there is not. The same patch puts the running build in the watermark: the organizer reported a fix as "still not working" while the build carrying it had just been published, and neither of us could tell which build their browser was actually running — the version lives only in the service worker's cache name. The tag reads the cache really serving the page, filtered to this site's own prefix so a browser that has visited both the league site and the test site never reports the other one's build. Test: `tabs/court-panel-title.spec.ts`.

- **p92 — a court change the organizer makes is not counted as a court drop** (the organizer, 22 September, asked whether a pre-session move should change the court a player earned: "Yes, moving a player rewrites the court they have earned, but only when a high caliber player comes, and its upto the admin to make that decision, so I can reorgranise the courts, it should not show as a drop in thier stats or anything"): measured before the fix, moving a player from Court 3 to Court 5 left their rating (1379), their wins and losses and their best court untouched — but their court-climb figure went from −1 to −3, recording a two-court drop for a night they had not played, because court climb is `first session's court − court now` and the organizer's move moves the second half of that subtraction. Every court change made by hand is now remembered as an adjustment (`admin_court_moves`: courts moved, down positive, up negative) and added back when the climb is worked out, so the figure keeps measuring what a player did on court — a courtesy move down costs them nothing and a move up hands them nothing. Test: `tabs/admin-move-not-a-drop.spec.ts`, including a case where a move by hand and two courts won by play are both accounted for.

- **p93 — the drag-and-drop court board works before a session, and shows what the organizer picks on** (the organizer: "dont I have the flexibility of moving players from one court to another? ... Is this the right place to do the court assignments? do a deep check", then "Yes — enable drag-and-drop before a session", and on how they choose: "I move a player down based on thier wins, if wins tie then look at the points scored not ELO"): Admin → Assign refused to work until a session had started, so the only way to arrange tonight's courts was the Courts tab's panel — two dialogs deep, one player at a time, with the line-up recomputed and the dialog closed after every move. The same board now arranges tonight's starting courts: drag between courts, or out to **Not playing** (which marks the player not coming, exactly as Remove does, keeping the court they earned). Every chip carries the two numbers the decision is made on — season wins and points scored, from the games themselves (`seasonRecord`) — and before a session each court lists its players in that order, so the one to move down is the one at the bottom. Elo is deliberately absent: it is not what this decision is made on. During a session the court keeps the order the night put it in, so nothing about live play changes. Tests: `tabs/assign-before-session.spec.ts`, and the generated `tabs/assign.spec.ts` now checks the pre-session board across its hundred leagues instead of asserting the old refusal.

- **p94 — the board's player chip fits a 320-pixel phone**: p93 added wins and points to every chip on the court board. The chip is a flex row inside a two-column grid, so on a 320-pixel screen it became name + record + court selector in about 145 pixels and the selector ran off the side of the page. The chip now wraps — name and record on the top line, the selector at the chip's own width below. Caught by the full suite's phone reflow check and **not** by the sanity suite, so the 320-pixel check is now part of sanity: it costs twelve seconds and would have caught this before the push.

- **p95 — the court panel stays open while the organizer arranges it, and says what it did** (the organizer, 22 September: "Courts Add player/remove is not working properly"): the panel closed on every click. Adding four players to a court meant opening the court, opening "+ Add Player to Court", choosing a player and saving — four times over — and removing a player closed it too, so seeing the result meant reopening the court. Removing also reported "Answer updated for Liam Leblanc", which is the wording of an RSVP change (what happens underneath) and said nothing about the court or about the player keeping the court they earned. The panel now reopens on the court being arranged with the line-up as it stands, and a removal says "X is off Court 3 for Session 2 — they keep Court 3". During a session nothing changes: the court engine puts its own preview dialog on screen there, and reopening the panel over it would take it away. Test: `tabs/court-panel-stays.spec.ts`, which adds three players in a row without reopening anything.

- **L27 — a court's scores no longer collide with another court's** (the organizer, 22 September: "app stopped working on round 2 again today in middle of session, was getting some database sync error"): diagnosed live on production, then reproduced on the rehearsal server before anything was written. Two faults, not one.
  **The wedge.** `save_court_scores` takes a league-wide advisory lock. L26 bounded how long a call may WAIT for it (5s) and RUN (20s), but not how long a connection may sit **idle inside its transaction** while holding it. A phone that dropped mid-request left its transaction open and held the lock from 20:57 until it was released by hand the next morning; every save in between queued, timed out and was retried for ever. `idle_in_transaction_session_timeout=15s` on the three app roles closes it (applied by hand on 23 September to stop the bleeding, and carried in this migration).
  **The collision**, which is why it kept happening even without a wedge: every court writes the same `current_session` row, and the function refused any save whose `p_expected` did not match the **whole league's** version. Two courts finishing a round together meant the first bumped the version and the second was refused. Reproduced exactly — four courts saving at the same instant, all holding the correct version: **one succeeded, three were refused, 3 of 12 scores stored**. The lock is now **per court** (`pg_advisory_xact_lock(7262026, p_court)`), the state is re-read inside that lock so each court merges into the latest value, and the league-wide version is no longer a conflict for a score save. The rules that matter are unchanged: the same night, the same round, the same line-up on that court, an unfinished session, only that court's players or the organizer, and every scoring rule.
  With the fix the same reproduction gives **4 of 4 courts saved, 12 of 12 scores**. Tests: `legacy/tests/db/concurrency.sh` — a real Postgres, real concurrent connections, run as part of `legacy/tests/db/run.sh`; `WITHOUT_L27=1` leaves the fix out and the suite fails (1/4 courts, 3/12 scores), which is how it was proved to catch the fault. `legacy/tests/db/gen_cases.py` was corrected to the new rule rather than around it, and `mock-supabase.ts` now mirrors the database so the app tests are not testing a fiction. `verify.sql` checks both halves on every release.

- **The full-night simulation** (`legacy/tests/db/night.sh`, asked for after 22 September: "simulate a night like this and then see, when multiple people try to use the app at the same time"): six courts, twenty-four players, two rounds, one save per game exactly as the app makes them, all six courts' phones saving at once, a phone dropped mid-round by ending whichever connection holds the league lock, and the organizer's page polling throughout. It then advances the round and ends the session through the real functions. **With L27**: 36 saves, 0 failures, 36/36 games stored, 48 organizer reads with none failed, slowest single save under full six-court contention **75 ms**, the night ended and filed, nothing left waiting on the lock. **With `WITHOUT_L27=1`**: 4 of 18 games in round 1, **32 of 36 saves refused**, the round advance refused with "Finish every court before advancing" and End Session refused with "Finish both rounds and every court before ending the session" — the night cannot be completed, which is exactly what happened on 22 September. Runs as part of `legacy/tests/db/run.sh`, which CI runs on every push (`.github/workflows/root-quality.yml`).
