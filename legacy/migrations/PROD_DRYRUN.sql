-- DRY RUN of PROD_2026-27.sql on the production database. Everything runs inside ONE transaction and the last
...
-- Contents: L01 auth+rules, L03 scores+season, L04 backfill, L05 rollover, L06 statistics, L07 reminders+spares, L08 payments+push, L09 invitations, L10 payment declaration + voting lock, L11 vote log (Sunday 10 PM deadline), L12 job grants, L13 undo, L14 instant reminders (needs the Vault secret github_dispatch_token), L15 hardening, L16 TEST and production identical, L17 past players.
...
-- ═══════════ L10_payment_declaration_and_vote_lock ═══════════
...
do $dry$ declare ok int; total int; bad text; begin
 select count(*) filter (where result='OK'), count(*), string_agg(check_name||': '||result, '; ') filter (where result<>'OK') into ok, total, bad from (select 'rls '||c.relname as check_name,case when c.relrowsecurity then 'OK' else 'FAIL: RLS off' end as result
...
union all
select 'player columns',case when count(*)=8 then 'OK' else 'FAIL: '||count(*)||'/8' end from information_schema.columns where table_schema='public' and table_name='players' and column_name in('approved','waitlisted','registered_at','admin_note','user_id','updated_at','email_reminders','membership_type')) v;
