...
// status instead of a button. During a session Call In seats the player through the court engine. Expected courts and
...
for (let i = 0; i < 12; i++) {
  test(`Call in ${String(i + 29).padStart(3, "0")} · during a session · the court engine seats the player or says why not`, async () => {
    ...
    const ref = reference({ ...adjustInput(cs0), absent: [], returning: [{ id: who!.id, court: who!.current_court || 6 }], late: [] }) as { ok: boolean };
    await openPlayers(page);
    await page.getByRole("button", { name: `Call in ${who!.name}` }).click();
    if (ref.ok) await expect.poll(() => courtOf(kvOf(ctx, "current_session").assignments, who!.id), { message: "seated tonight" }).toBeGreaterThan(0);
