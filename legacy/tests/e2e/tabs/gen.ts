...
export function genLeague(seed: number, o: GenOpts = {}): League {
  ...
  for (const p of players) { const rows = payments.filter((x) => x.player_id === p.id); p.paid = p.membership_type === "spare" ? rows.filter((x) => x.kind === "spare"&&x.session_number===U).reduce((n,x)=>n+x.amount,0)>=20 : rows.filter((x) => x.kind === "season" || x.kind === "adjustment").reduce((n, x) => n + x.amount, 0) >= 400; }
