...
// starting-courts reference) or standby. During a session a player already seated tonight shows "✓ Playing · Court N"
// (p66: the organizer's Call Ins had worked, but the pool kept showing Call In). Everyone else has Call In, shown
// disabled with the reason once both rounds are finished.
...
export const POOL_ROW = "div:has(> button[onclick^='callInSpare']), div:has(> span.tag[title*=' is coming to the next session']), div:has(> span.tag[title*=' is playing tonight'])";

/** What the pool shows for a player: "✓ Playing · Court N", "✓ Coming · Court N", "Coming · standby" or "📲 Call In". */
export function poolAction(ctx: Ctx, L: League, id: number): string {
  if (L.current) { const c = courtOf(L.current.assignments, id); return c ? `✓ Playing · Court ${c}` : "📲 Call In"; }
